...
function DoomRenderer({ appCoreRef }) {
	...
	useEffect(() => {
		// Same-origin bundle avoids CORS: cdn.dos.zone does not send Access-Control-Allow-Origin for
		// localhost / arbitrary sites, which makes fetch() fail with "Failed to fetch".
		const pub = (process.env.PUBLIC_URL || "").replace(/\/$/, "");
		const jsdosUrl = pub ? `${pub}/doom.jsdos` : "/doom.jsdos";
